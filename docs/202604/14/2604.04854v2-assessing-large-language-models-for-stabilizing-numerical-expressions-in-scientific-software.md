...
## 摘要
科学软件依赖高精度计算，然而有限的浮点表示可能会引入精度误差，并在安全关键领域中传播。尽管大语言模型（LLMs）在科学应用中的使用日益增多，但它们在处理浮点数值稳定性方面的可靠性尚未得到系统评估。本文通过两项数值稳定化任务评估了 LLM 在高精度数值计算上的推理能力：(1) 通过生成诱导误差的输入来检测数值表达式中的不稳定性（检测）；(2) 重写表达式以提高数值稳定性（稳定化）。利用流行的数值基准测试，我们在近 2,470 个数值结构上评估了六种 LLM，这些结构包括嵌套条件语句、高精度字面量和多变量算术。研究结果表明，在检测和稳定数值不稳定的计算方面，LLM 与最先进的传统方法同样有效。更值得注意的是，LLM 在基准方法失败的情况下表现更优：在基准方法未能提高精度的 17.4% (431) 的表达式中，LLM 成功稳定了其中的 422 个 (97.9%)；并且在所有表达式中，有 65.4% (1,615) 的表达式通过 LLM 获得了比基准方法更高的稳定性。然而，LLM 在处理控制流和高精度字面量时表现不佳，往往会移除这些结构，而不是推理其数值含义，相比之下，它们在纯符号表达式上的表现要好得多。总之，这些发现表明 LLM 能够有效稳定经典技术无法处理的表达式，但在必须对精确数值大小和控制流语义进行精确推理时仍面临挑战，因为这类具体模式在训练过程中很少遇到。
