...
## 摘要
深度神经网络已成为学习定义在无限维函数空间上的算子的强大工具。然而，现有理论经常面临与维度和有限可解释性相关的困难。本研究探讨了稀疏性如何帮助解决泛函学习中的这些挑战，而泛函学习是算子学习的核心组成部分。我们提出了一个框架，利用卷积架构从有限数量的样本中提取稀疏特征，并结合深度全连接网络来有效地逼近非线性泛函。利用通用离散化方法，我们证明了稀疏逼近器能够从离散样本中实现稳定恢复。此外，确定性采样和随机采样方案对于我们的分析都是充分的。这些发现提高了在各种函数空间（包括具有快速频率衰减和混合光滑性的空间）中的逼近率并减少了样本量。它们还为稀疏性如何缓解泛函学习中的维度灾难提供了新的理论见解。
