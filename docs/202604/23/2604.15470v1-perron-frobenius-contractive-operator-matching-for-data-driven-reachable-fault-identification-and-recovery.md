...
tags: ["query:pde-llm"]
score: 7.0
evidence: 用于系统识别的福克-普朗克方程和算子匹配
tldr: 本文针对非线性控制仿射系统的执行器故障，提出了一种基于数据驱动的故障检测、识别与恢复（FDIR）框架。该方法利用Perron-Frobenius（PF）算子在概率密度空间建模，通过流映射匹配（FMM）从轨迹数据中学习故障索引算子，并结合收缩性证明建立2-Wasserstein度量下的误差界限。该框架支持连续参数空间的在线故障拟合及分布外（OOD）泛化，并通过高斯混合协方差控制实现恢复，在航天器姿态控制仿真中验证了其有效性。
source: arxiv
selection_source: fresh_fetch
motivation: 传统的基于轨迹的故障诊断难以处理非线性系统中的不确定性，且缺乏对故障可检测性和可识别性的定量理论保证。
method: 利用PF算子在密度空间表征状态演化，通过流映射匹配学习算子库，并协同训练收缩证书以界定近似误差。
result: 建立了基于2-Wasserstein距离的故障辨识界限，实现了对连续故障参数的在线拟合及分布外场景的有效恢复。
conclusion: 该框架为非线性系统的FDIR提供了一种具有理论误差保证且计算高效的数据驱动解决方案。
---

## 摘要
本文研究执行器故障下非线性控制仿射系统的数据驱动故障检测、识别与恢复 (FDIR)。我们利用故障索引的 Perron-Frobenius (PF) 算子，在概率密度空间（而非单个轨迹）中构建了一个统一框架，用于预测不同故障剖面下状态分布的演化。通过利用 Fokker-Planck 方程的概率流表示，我们构建了能够重现精确随机边缘分布的确定性 PF 算子，定义了前向可达密度族，并建立了故障驱动演化与名义密度演化之间散度的可证明 2-Wasserstein 界限。这些为各种故障的可检测性和可识别性提供了定量条件。故障索引算子通过流映射匹配 (FMM) 从轨迹数据中学习，我们证明了可观测的 FMM 残差直接界定了算子在 2-Wasserstein 度量下的逼近误差。此外，我们共同训练了一个收缩证明（contraction certificate），用于界定学习到的算子族、实际故障驱动密度流以及名义动力学之间的差距。随后，该算子库被用于在线连续参数空间内的连续故障参数拟合，以将学习到的算子推广到分布外 (OOD) 场景。为了执行恢复控制，我们采用了可达密度传播和高斯混合协方差控制。所提出的框架在具有四个反作用轮的 10 状态航天器姿态控制系统上得到了验证。
