...
## 摘要
光学化学结构识别 (OCSR) 对于将印刷文献中的二维分子图示转换为机器可读格式至关重要。尽管视觉语言模型在端到端 OCR 任务中展现出潜力，但将其直接应用于 OCSR 仍具挑战性，且直接进行全参数监督微调往往会失败。在这项工作中，我们将该任务表述为以图像为条件的 SMILES 生成，从而使 DeepSeek-OCR-2 适配于分子光学识别。为了克服训练不稳定性，我们提出了一种两阶段渐进式监督微调策略：从参数高效的 LoRA 开始，逐步过渡到使用分层学习率的选择性全参数微调。我们在结合了来自 PubChem 的合成渲染图和来自 USPTO-MOL 的真实专利图像的大规模语料库上训练模型，以提高覆盖范围和鲁棒性。我们微调后的模型 MolSeek-OCR 展示了极具竞争力的能力，实现了与性能最佳的图像到序列（image-to-sequence）模型相当的精确匹配准确率。然而，它仍逊于目前最先进的图像到图（image-to-graph）模型。此外，我们探索了强化学习风格的后训练和基于数据治理的精炼，发现它们未能提高精确 SMILES 匹配所需的严格序列级保真度。
