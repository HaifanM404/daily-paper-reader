...
title_zh: 置信度是关键：符合性预测如何增强渗透性多肽的生成式设计
...
tags: ["query:ddi"]
score: 6.0
evidence: 用于从头分子设计和化学空间探索的预测模型
tldr: 本研究针对生成模型在设计渗透性环肽时，预测模型因超出适用范围导致结果不可靠的问题，提出了一种结合强化学习与保形预测（CP）的生成框架。通过集成CP量化不确定性，该方法能在特定置信水平下评估设计。实验证明，利用CP校准的预测作为奖励，显著提升了肽链优化的效率与可靠性，并有效约束了模型在可靠化学空间内进行探索，为大分子药物设计提供了新路径。
source: arxiv
...
motivation: 强化学习在分子设计中常因预测模型超出适用范围而产生不可靠结果，导致设计进入高奖励但高不确定性的区域。
method: 在强化学习引导的生成框架中集成保形预测（CP）技术，构建不确定性感知的渗透性预测器作为评分组件。
result: 采用CP校准的奖励机制提高了环肽优化的效率和可靠性，并成功阻止了模型向预测器适用范围外的盲目探索。
conclusion: 首次将保形预测与生成模型结合，有效解决了预测不确定性问题，为开发具有高渗透性的环肽药物提供了稳健的计算工具。
---

## 摘要
生成模型与强化学习（RL）相结合（如 REINVENT 和 PepINVENT）已成为从头分子设计的强大框架。在构思过程中，这些生成框架利用各种预测模型作为优化目标的一部分。然而，预测模型的效用可能受其适用范围（domain of applicability）的限制。当使用 RL 结合预测模型探索化学空间时，它可能会建议位于预测模型适用范围之外的分子。结果，预测可能变得不那么可靠，从而可能将设计引导至高奖励但高不确定性的化学空间。这种情况在环肽中尤为突出；环肽因其可修饰性和巨大的相互作用表面而具有治疗前景，但与小分子相比，其研究尚不充分。虽然环肽的被动膜渗透已引起关注，但识别最佳渗透性设计对于靶向细胞内位点仍然具有挑战性且至关重要。我们提出了一种 RL 引导的生成框架，该框架使用具有不确定性感知能力的渗透性预测器作为评分组件来设计渗透性环肽。为了解决预测不确定性（特别是受新颖化学结构影响的不确定性），我们集成了符合性预测（CP）作为我们的不确定性量化方法。CP 在用户定义的置信水平下，基于校准模型对设计进行评估。我们证明，利用基于 CP 的预测结果对生成的肽进行奖励，可以提高肽优化过程的可靠性和效率。这也抑制了在预测器适用范围之外的探索。该方法弥合了预测不确定性与 RL 引导探索之间的差距，首次展示了如何将生成建模与符合性预测相结合。
