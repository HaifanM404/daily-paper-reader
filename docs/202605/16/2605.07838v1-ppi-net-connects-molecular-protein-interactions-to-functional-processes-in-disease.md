...
tags: ["query:ddi"]
score: 7.0
evidence: 用于蛋白质相互作用和疾病建模的图神经网络
tldr: "PPI-Net是一种分层图神经网络，旨在解决分子改变如何驱动疾病的难题。它将蛋白质相互作用（PPI）网络与Reactome通路层级相结合，通过图注意力机制将基因级信号聚合为高阶生物程序。在TCGA多种癌症数据上的实验表明，该模型不仅预测准确率高（部分超过90%），还能通过多组学整合揭示关键致癌模块，为癌症生物学提供机械性见解。"
source: arxiv
...
motivation: 现有的高通量分析模型往往忽略了结构化的生物学关系，且在跨尺度解释性方面存在不足。
method: 开发了PPI-Net模型，利用图注意力机制将患者分子谱在STRING相互作用网络中嵌入，并沿Reactome通路层级进行传播与聚合。
result: 在TCGA的10种癌症预测中表现优异，且相比单一PPI模型或单层监督模型，其分层架构显著提升了预测准确率。
conclusion: 将相互作用网络与通路层级相结合，能够实现高精度的疾病预测，并为理解癌症的分子机制提供可解释的生物学视角。
---

## 摘要
理解分子改变如何在生物系统中传播以驱动疾病仍然是一个核心挑战。尽管高通量分析能够全面表征肿瘤状态，但大多数模型忽略了结构化的生物学关系，或缺乏跨尺度的可解释性。在此，我们提出了 PPI-Net，这是一种分层图神经网络，它将蛋白质-蛋白质相互作用 (PPI) 网络与通路级表示相结合，从而实现从分子相互作用到功能过程的疾病建模。患者特异性的分子谱被嵌入到来自 STRING 的共享相互作用网络中，并通过使用图注意力机制的多层 Reactome 层级结构进行传播，从而能够将基因级信号聚合为更高阶的生物程序。在来自癌症基因组图谱 (TCGA) 的十种癌症类型的 RNA-seq 数据中，PPI-Net 实现了稳健的预测性能，在多个队列中的平衡准确率超过 90%。对乳腺癌 RNA-Seq 数据的对比分析表明，PPI-Net 对 Reactome 层级结构的整合相对于仅使用 PPI 的模型将平衡准确率提高了 6.7%，而分层多级监督相对于仅使用单个顶层预测头将平衡准确率提高了 12.3%。应用结合 RNA-seq 和甲基化数据的多组学方法改进了模型解释，恢复了包括 TP53-AKT 信号传导和应激反应通路在内的经典致癌模块，同时揭示了其向离子信号传导和细胞对刺激反应等一致程序的收敛。这些结果表明，将相互作用网络与通路层级结构相结合，在实现准确预测的同时，也为癌症生物学提供了机制性的见解。
